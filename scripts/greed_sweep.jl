# Greed sweep: everyone greedy, the hoarding slider from 0 (all consumption) to 1 (all capital), both systems; plus the
# land-scaled village with rationing at four loaves. 6 seeds, 50 rounds.
...
variants = Vector{Tuple{String, NamedTuple}}([("no greed", (;)), ("no greed, indexed income", (; guaranteed_income_in_breads = 1.0))])
for h in (0.0, 0.5)
    push!(variants, ("hoarding $(h)", (; greed = true, greed_share = 1.0, greed_hoarding = h)))
    push!(variants, ("hoarding $(h), indexed income", (; greed = true, greed_share = 1.0, greed_hoarding = h, guaranteed_income_in_breads = 1.0)))
end
out = DataFrame[]
for (name, kw) in variants, (system, base) in (("debt", DEBT), ("sumsy", SUMSY)), s in 1:nseeds
    ...
    push!(out, d); println(name, " ", system, " seed ", s, " alive ", d.persons_alive[end], " rounds ", nrow(d)); flush(stdout)
end
CSV.write(joinpath(@__DIR__, "..", "results", "indexed_gi_rounds.csv"), vcat(out...; cols = :union))
