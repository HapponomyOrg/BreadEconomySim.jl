...
## 7c. Supplemental transitions: cooperatives (decided 25 September)
- **Starting village and target on realistic price rules**: the ladder was redefined on 25 September (the realistic price rules from step 0; old step 10 removed, steps renumbered: the reserve is step 10, the fiscal policy step 11). Starting villages and target are chosen again from the 25 September results — the consistent pairing (same ownership) is the chosen debt step → its SuMSy twin with levy and land dividend (LD).
- **Co-ops founded at the switch**: when the transition starts, villagers found one cooperative of each kind (farm, bakery, theatre) under the founding rules; the existing firms keep going.
- **Market entry** (design_market_entry_2026-09-25): a variant of the transition with entry on and half the entrants cooperatives.
